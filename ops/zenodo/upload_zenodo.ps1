...
<p><b>Contents</b> (7.9 GB; every cell holds <code>result.h5</code> with samples, weights, log-densities and diagnostics, for MoleWhacker the iteration log and the stored final mixture, plus <code>metadata.json</code> and <code>summary.json</code>):</p>
<ul>
